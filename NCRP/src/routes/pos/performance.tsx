...
import { formatNumber, monthlySeries, stateRows, posTiles } from "@/lib/telecom";
import { Gauge, Timer, TrendingUp } from "lucide-react";

export const Route = createFileRoute("/pos/performance")({
  head: () => ({
    meta: [
      { title: "PoS Performance Report | NCRP Telecom Intelligence" },
      { name: "description", content: "Blacklisting, TSP complaints and FIR outcomes across the PoS repository." },
      { property: "og:title", content: "PoS Performance Report | NCRP Telecom Intelligence" },
      { property: "og:description", content: "Blacklisting, TSP complaints and FIR outcomes across the PoS repository." },
    ],
  }),
  component: PosPerformance,
});
...
function PosPerformance() {
  ...
  return (
    <>
      <PageHeader
        title="PoS Performance Report"
        description="Blacklisting, TSP complaints and FIR outcomes across the PoS repository."
        crumbs={[{ label: "Telecom Module" }, { label: "SIM Point of Sale" }, { label: "Performance Report" }]}
      ...
      <div className="mb-6 grid gap-4 xl:grid-cols-2">
        <Panel className="overflow-hidden">
          ...
          <TrendLine data={monthlySeries("PosPerformance", posTiles[0].value)} keys={[
